...
local defaultSizeX = 1250
local defaultSizeY = 900
...
--mdc: SetAxisOrientation(true, true)
...
local function open(name)
  ...
  -- OR create a new one now
    ...
    name,
    wx.wxPoint(screenSizeX - defaultSizeX, screenSizeY - defaultSizeY),
    wx.wxSize(defaultSizeX, defaultSizeY),
    wx.wxDEFAULT_FRAME_STYLE + wx.wxSTAY_ON_TOP
    - wx.wxRESIZE_BORDER - wx.wxMAXIMIZE_BOX)
  frame:SetClientSize(defaultSizeX, defaultSizeY)
...
local drawing = {
  ...
  zero = function (x, y)
    ...
    -- mdc:SetAxisOrientation(false, false)
    -- mdc:SetUserScale(0.5,0.5)
  end
}
